        ...
        **kwargs,
        ...
        Args:
            ...
            kwargs:
                Additional keyword arguments used to hide deprecated arguments
        """
        assert isinstance(self.train_dataset, TTTOfflineLoopDataset)
        resume_from_checkpoint = None if not resume_from_checkpoint else resume_from_checkpoint
        ...
        for epoch in range(epochs_trained, num_train_epochs):
            ...
            for step, list_of_inputs in enumerate(epoch_iterator):
                # Skip past any already trained steps if resuming training
                if steps_trained_in_current_epoch > 0:
                    steps_trained_in_current_epoch -= 1
                    if steps_trained_progress_bar is not None:
    ...
    def evaluation_loop(
        ...
        if (
            ...
            if self.args.train_data_source == "stream":
                preds, initial_predictions = compute_metrics_fn(
            ...
            metrics = preds if isinstance(preds, dict) else None
            if metrics is not None:
                for key in list(metrics.keys()):
                    if not key.startswith(f"{metric_key_prefix}_"):
                        metrics[f"{metric_key_prefix}_{key}"] = metrics.pop(key)

            return EvalLoopOutput(predictions=preds, label_ids=None, metrics=metrics, num_samples=1)
